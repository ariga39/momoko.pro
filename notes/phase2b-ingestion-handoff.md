...
- 独占文件：
  ...
  - `schemas/source.schema.json`（显式 robots_result/robots_path_decision 与 path-bound evidence；旧 robots_approved 仅兼容读取）
...
- cron 门：`runCron()` 只跑 `automated_fetch=true` 且请求路径通过 robots pure decision 的来源；
