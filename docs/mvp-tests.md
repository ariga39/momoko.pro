...
- [ ] **schema 正/反实例全量校验**：每 canonical 类型（content/locale/discovery/anniversary/retraction/manifest/source）至少各 1 正例+1 反例，含 published 拒绝、reviewed 无 reviewer/time 拒绝、uri/date-time format 拒绝（严格 RFC3339：date-only/无时区/非法日拒绝，hostname 拒绝 `-x`/`..`）、`robots_result`/`robots_path_decision` 结果枚举与 path-bound evidence、非法 result/path 组合拒绝、automated_fetch=true 需非 manual 频率且声明 `fetch_paths`（false 需 manual）；`tools/schema/validate_schemas.py` 用真实 FormatChecker 通过
