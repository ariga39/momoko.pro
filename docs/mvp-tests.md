...
- [ ] **schema 正/反实例全量校验**：每 canonical 类型（content/locale/discovery/anniversary/retraction/manifest/source）至少各 1 正例+1 反例，含 published 拒绝、reviewed 无 reviewer/time 拒绝、uri/date-time format 拒绝（严格 RFC3339：date-only/无时区/非法日拒绝，hostname 拒绝 `-x`/`..`）、`access_control`/`terms_status` 枚举、`robots_result`/`robots_path_decision` 结果枚举与 path-bound evidence、非法 result/path 组合拒绝、automated_fetch=true 需非 manual 频率且声明 `fetch_paths`（false 需 manual）；`tools/schema/validate_schemas.py` 用真实 FormatChecker 通过
...
- [ ] **cron seam**：cron 只运行 `automated_fetch=true`、声明非空 `fetch_paths` 且每个请求路径通过 pure robots decision 的来源 adapter；同一显式路径传入 adapter，无默认 `/`；false 来源只走 manual-import；无允许来源/无变化静默；当前 S1–S5 全 false 时 cron 无输出
