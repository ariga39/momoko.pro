...
- 改变条件：用户明确需要且人工批准；或某来源项目门置 `automated_fetch=true` 并为目标路径取得 robots 结果/decision 证据后开放自动抓取（届时才引入抓取链契约）。人类明确指令下的单页读取另行适用访问控制、Terms、限频和内容保留边界，不因 robots 404 自动拒绝，也不因 robots 允许自动取得再利用许可。
